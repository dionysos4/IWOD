...
def main():
    ...
    val_dataset = lcod.LCDDataset(cfg["data_directory"], "valid", transform=transform_val, depth=cfg["depth_type"])
    ...
    checkpoint_callback_val = ModelCheckpoint(
                            ...
                            filename="lcod-{epoch:02d}-{val_loss:.2f}",
    )
    ...
    checkpoint_callback_train = ModelCheckpoint(
                            ...
                            mode="min",
                            dirpath=checkpoint_dir,
                            filename="lcod-{epoch:02d}-{train_loss:.2f}",
    )
    ...
    trainer = L.Trainer(callbacks=[checkpoint_callback_val, checkpoint_callback_train, early_stop_callback], num_sanity_val_steps=cfg["num_sanity_val_steps"], 
                      accelerator=cfg["accelorator"], devices=cfg["gpu_devices"], strategy=cfg["strategy"], num_nodes=cfg["num_nodes"], precision=cfg["precision"], log_every_n_steps=cfg["log_every_n_steps"],
